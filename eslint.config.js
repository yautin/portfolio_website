import js from '@eslint/js'
import globals from 'globals'
import reactHooks from 'eslint-plugin-react-hooks'
import reactRefresh from 'eslint-plugin-react-refresh'
import tseslint from 'typescript-eslint'
import { defineConfig, globalIgnores } from 'eslint/config'

export default defineConfig([
  // dist = build output; contracts = Foundry/Solidity; supabase = Deno edge
  // functions (linted by Deno, not the app's ESLint — they use npm: imports).
  globalIgnores(['dist', 'contracts', 'supabase']),
  {
    files: ['**/*.{js,jsx}'],
    extends: [
      js.configs.recommended,
      reactHooks.configs.flat.recommended,
      reactRefresh.configs.vite,
    ],
    languageOptions: {
      globals: globals.browser,
      parserOptions: { ecmaFeatures: { jsx: true } },
    },
  },
  // Build-tooling config that runs in Node, not the browser (vite.config.js
  // reads process.cwd() for the feature banner). Without this it inherits the
  // browser globals above and `process` reads as undefined.
  {
    files: ['*.config.js'],
    languageOptions: { globals: globals.node },
  },
  // Strict-typed Web3 modules (src/web3/**).
  {
    files: ['**/*.{ts,tsx}'],
    extends: [
      js.configs.recommended,
      ...tseslint.configs.recommended,
      reactHooks.configs.flat.recommended,
      reactRefresh.configs.vite,
    ],
    languageOptions: {
      globals: globals.browser,
      parserOptions: { ecmaFeatures: { jsx: true } },
    },
  },
])
